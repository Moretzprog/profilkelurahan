...
$(document).ready(function() {
    $('#formSaran').submit(function(e) {
        ...
        $.ajax({
            ...
            error: function() {
                $('#response').html('<div class="alert alert-danger">Terjadi kesalahan, silakan coba lagi.</div>');
            }
        });
    });
});
...
// ===== Load Saran Admin =====
$(document).ready(function() {
    function loadAdminSaran() {
        $.ajax({
            url: 'pages/load_saran_admin.php',
            type: 'GET',
            success: function(data) {
                $('#adminSaranList').html(data);
            },
            error: function() {
                $('#adminSaranList').html('<div class="list-group-item text-danger">Gagal memuat saran.</div>');
            }
        });
    }

    $('#saranModal').on('shown.bs.modal', function () {
        loadAdminSaran();
    });

    setInterval(function() {
        if($('#saranModal').hasClass('show')) {
            loadAdminSaran();
        }
    }, 5000);
});
...
function saveContactChanges() {
    ...
    var telepon = $('#inputPhone').val();
    var email = $('#inputEmail').val();

    $.ajax({
        url: 'pages/update_contact.php',
        type: 'POST',
        data: { alamat: alamat, telepon: telepon, email: email },
        success: function(response) {
            // Update DOM kontak
            $('#contact-address').html('<i class="fas fa-map-marker-alt me-2"></i> ' + alamat);
            $('#contact-phone').html('<i class="fas fa-phone me-2"></i> ' + telepon);
            $('#contact-email').html('<i class="fas fa-envelope me-2"></i> ' + email);
            ...
            // ✅ PENTING: Tunggu animasi modal selesai baru bersihkan
            setTimeout(function() {
                // Hapus semua backdrop yang mungkin tersisa
                $('.modal-backdrop').remove();
                
                // Reset body ke kondisi normal
                $('body').removeClass('modal-open');
                $('body').removeAttr('style'); // Hapus semua inline style
                
                // Tampilkan toast
                showToast('Informasi kontak berhasil diperbarui!', 'success');
            }, 500); // Tunggu 500ms untuk animasi fade selesai

            console.log(response);
        },
        error: function() {
            showToast('Gagal memperbarui informasi kontak.', 'danger');
        }
    });
}

// Load kontak ke modal saat dibuka
$('#editContactModal').on('show.bs.modal', function() {
    $.getJSON('pages/get_contact.php', function(data) {
        $('#inputAddress').val(data.alamat);
        $('#inputPhone').val(data.telepon);
